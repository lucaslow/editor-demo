...
export const drawVerticalRuler = (ctx, start, options) => {
  const { scale, width, height, canvasConfigs } = options
  const { bgColor, fontColor, longfgColor, shortfgColor } = canvasConfigs
  ...
  for (
    ...
    ctx.moveTo(0, y)
    ctx.save() // 这里先保存一下状态
    ctx.translate(width * 0.5, y + 8) // 将原点转移到当前画笔所在点
    ctx.rotate(-Math.PI / 2) // 旋转 -90 度
    ctx.scale(-FONT_SCALE, FONT_SCALE) // 缩放至10px
    ctx.fillText(value, -4, 7) // 绘制文字
    // 回复刚刚保存的状态
    ctx.restore()
