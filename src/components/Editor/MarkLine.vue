...
export default {
  ...
  mounted() {
    ...
    eventBus.$on('move', (dragNode, isDownward, isRightward) => {
      if (dragNode.className.includes('shape-multiple')) {
        this.isCtrl = true
      } else {
        this.isCtrl = false
      }
  ...
  methods: {
    ...
    showLine(dragNode, isDownward, isRightward) {
      ...
      // const reScaleRatio = 1 / this.canvasData.scaleRatio
      const components = document.querySelectorAll(
        '.shape:not(.editor-shape-selected)'
      )
      ...
      components.forEach(node => {
        ...
        Object.keys(conditions).forEach(key => {
          ...
          conditions[key].forEach(condition => {
            ...
            // 修改当前组件位移
            !this.isCtrl &&
              this.$store.commit('Canvas/setShapePosStyle', {
                key,
                value: condition.dragShift
              })
            this.isCtrl &&
              this.$store.commit('Canvas/CtrlComponent/setCtrlShapePosStyle', {
                key,
                value: condition.dragShift
              })
